...
/*
 * Class that makes it easy to drop debug visualizations inline with code that may need it
 */
...
public static class DebugDraw
{
  ...
  static readonly Mesh solidCube;
  static Material activeMaterial;
  static Material ignoreDepthMaterial;
  static Material unlitColorMaterial;

  static DebugDraw() {
    solidSphere = creator.CreateSphere(3);
    solidCube = creator.CreateCube(0);
    unlitColorMaterial = new Material(Shader.Find("Unlit/Color"));
    ignoreDepthMaterial = new Material(Shader.Find("Custom/IgnoreDepth"));
    activeMaterial = ignoreDepthMaterial;
  }
    ...
    public static void DrawSphere(Vector3 position, float radius, Color color)
    {
        ...
        Graphics.DrawMesh(solidSphere, mat, activeMaterial, 0, null, 0, block);
    }
    ...
    public static void DrawCube(Vector3 position, Quaternion rotation, float size, Color color)
    {
        ...
        MaterialPropertyBlock block = new MaterialPropertyBlock();
        block.SetColor("_Color", color);
        Graphics.DrawMesh(solidCube, mat, activeMaterial, 0, null, 0, block);
    }
